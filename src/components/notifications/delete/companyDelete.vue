<template>
  ...
  >
    <h1 class="text-2xl"><b>Ar tikrai norite ištrinti įmonę?</b></h1>
    <h1 class="text-xl break-words w-full">
      Įmonės pavadinimas: {{ popUpComponent.data.name }}
    </h1>
...
export default {
  methods: {
    ...mapActions(["deleteCompany", "setPopUpComponent"]),
    handleNe() {
      this.setPopUpComponent({});
    },
    handleTaip() {
      this.deleteCompany(this.popUpComponent.data.id);
    },
  },
  computed: {
